...
class _ProfileSettingsScreenState extends State<ProfileSettingsScreen> {
  ...
  Future<void> _handleSyncNow() async {
    ...
    final docs = await DocumentStorageService.loadSavedDocuments();
    final notes = await DocumentStorageService.loadHandwritingNotes();
    if (mounted) {
        ...
        SnackBar(
          content: Row(
            children: [
              ...
              Expanded(
                child: Text(
                  'Cloud Backup complete! Synced ${docs.length} ${docs.length == 1 ? 'file' : 'files'} & ${notes.length} ${notes.length == 1 ? 'note' : 'notes'}.',
              ...
              Expanded(
                child: Column(
                  ...
                  children: [
                    ...
                    Text(
                      isCloud
                          ? (user.email ?? 'Cloud Authenticated')
                          : 'Real-time sync enabled for ${user.name}',
                      style: const TextStyle(
                        fontSize: 11.5,
